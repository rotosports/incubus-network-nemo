...
func (suite *msgServerTestSuite) TestMintDepositAndWithdrawBurn_TransferEntireBalance() {
	...
	deposit := suite.QueryEarn_VaultValue(user, "bnemo")
...
func (suite *msgServerTestSuite) setupEarnForDeposits(valAddr sdk.ValAddress) string {
	suite.CreateVault("bnemo", earntypes.StrategyTypes{earntypes.STRATEGY_TYPE_SAVINGS}, false, nil)
	derivativeDenom := fmt.Sprintf("bnemo-%s", valAddr)
