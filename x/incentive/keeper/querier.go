...
func queryGetAPYs(ctx sdk.Context, req abci.RequestQuery, k Keeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	...
	// bnemo APY (staking + incentive rewards)
	...
	for _, param := range params.EarnRewardPeriods {
		// Skip bnemo as it's calculated earlier with staking rewards
...
func GetStakingAPR(ctx sdk.Context, k Keeper, params types.Params) (sdk.Dec, error) {
	...
	// Get incentive APR
	bnemoRewardPeriod, found := params.EarnRewardPeriods.GetMultiRewardPeriod(liquidtypes.DefaultDerivativeDenom)
	if !found {
		// No incentive rewards for bnemo, only staking rewards
		return stakingAPR, nil
	}

	// Total amount of bnemo in earn vaults, this may be lower than total bank
	// supply of bnemo as some bnemo may not be deposited in earn vaults
	...
	// Override collateral type to use "nemo" instead of "bnemo" when fetching
	incentiveAPY, err := GetAPYFromMultiRewardPeriod(ctx, k, types.BondDenom, bnemoRewardPeriod, totalEarnBnemoDeposited)
	if err != nil {
...
func getMarketID(denom string) string {
	...
	// e.g. "unemo" -> "nemo", "erc20/multichain/usdc" -> "usdc"
	// bnemo is not included as it is handled separately
