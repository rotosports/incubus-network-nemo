...
func (suite *KeeperTestSuite) TestMintDerivative() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			derivative := sdk.NewCoins(sdk.NewCoin(fmt.Sprintf("bnemo-%s", valAddr), tc.expectedDerivatives))
...
func (suite *KeeperTestSuite) TestIsDerivativeDenom() {
	...
	testCases := []struct {
		...
		{
			name:        "invalid - plain bnemo",
			denom:       "bnemo",
			wantIsDenom: false,
		},
		{
			name:        "invalid - bnemo prefix",
			denom:       "bnemo-",
			wantIsDenom: false,
		},
	}
...
func (suite *KeeperTestSuite) TestDerivativeFromTokens() {
	...
	suite.NoError(err)
	expected := sdk.NewCoin(fmt.Sprintf("bnemo-%s", valAddr), initialBalance)
