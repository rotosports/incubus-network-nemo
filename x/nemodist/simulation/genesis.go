...
// RandomizedGenState generates a random GenesisState for nemodist module
func RandomizedGenState(simState *module.SimulationState) {
	...
	nemodistGenesis := types.NewGenesisState(params, types.DefaultPreviousBlockTime)
	if err := nemodistGenesis.Validate(); err != nil {
		panic(err)
	}

	bz, err := json.MarshalIndent(&nemodistGenesis, "", " ")
	if err != nil {
		panic(err)
	}
	fmt.Printf("Selected randomly generated %s parameters:\n%s\n", types.ModuleName, bz)
	simState.GenState[types.ModuleName] = simState.Cdc.MustMarshalJSON(nemodistGenesis)
}
