...
Example:
$ %s query %s cdp fury15qdefkmwswysgg4qxgqpqr35k3m49pkxxvsmkx atom-a
...
$ kvcli q cdp cdps --owner=fury1hatdq32u5x4wnxrtv5wzjzmq49sxgjgs73xmvn
...
Example:
$ %s query %s deposits fury15qdefkmwswysgg4qxgqpqr35k3m49pkxxvsmkx atom-a
`, version.AppName, types.ModuleName)),
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
