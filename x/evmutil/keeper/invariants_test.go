...
func (suite *invariantTestSuite) SetupValidState() {
	...
	suite.FundModuleAccountWithNemo(
		types.ModuleName,
		sdk.NewCoins(
			sdk.NewCoin("unemo", sdkmath.NewInt(2)), // ( sum of all minor balances ) / conversion multiplier
		),
	)
...
func (suite *invariantTestSuite) TestSmallBalances() {
	...
	// add same number of unemo to avoid breaking other invariants
...
func (suite *invariantTestSuite) TestSendToModuleAccountNotAllowed() {
	...
	suite.ErrorContains(err, "nemo1w9vxuke5dz6hyza2j932qgmxltnfxwl78u920k is not allowed to receive funds: unauthorized")
}
