...
// GetBalance returns the total balance of anemo for a given account by address.
...
// SetBalance sets the total balance of anemo for a given account by address.
...
// SendBalance transfers the anemo balance from sender addr to recipient addr.
func (k Keeper) SendBalance(ctx sdk.Context, senderAddr sdk.AccAddress, recipientAddr sdk.AccAddress, amt sdkmath.Int) error {
	if amt.IsNegative() {
		return fmt.Errorf("cannot send a negative amount of anemo: %d", amt)
	}
...
// AddBalance increments the anemo balance of an address.
...
// RemoveBalance decrements the anemo balance of an address.
