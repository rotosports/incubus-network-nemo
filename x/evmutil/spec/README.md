...
The purpose of the `EvmBankKeeper` is to allow the usage of the `atfury` balance on the EVM via an account's existing `ufury` balance. This is needed because the EVM gas token use 18 decimals, and since `ufury` has 6 decimals, it cannot be used as the EVM gas denom directly.
