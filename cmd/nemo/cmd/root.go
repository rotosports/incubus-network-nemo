...
import (
	...
	nemoclient "github.com/incubus-network/nemo/client"
)

// EnvPrefix is the prefix environment variables must have to configure the app.
const EnvPrefix = "NEMO"

// NewRootCmd creates a new root command for the nemo blockchain.
func NewRootCmd() *cobra.Command {
	...
	rootCmd := &cobra.Command{
		Use:   "nemo",
		Short: "Daemon and CLI for the Nemo blockchain.",
...
// addSubCmds registers all the sub commands used by nemo.
func addSubCmds(rootCmd *cobra.Command, encodingConfig params.EncodingConfig, defaultNodeHome string) {
	...
	rootCmd.AddCommand(
		newQueryCmd(),
		newTxCmd(),
		nemoclient.KeyCommands(app.DefaultNodeHome),
	)
}
