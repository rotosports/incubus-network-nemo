...
HTTPS_GIT := https://github.com/Rotosports/fury.git
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/rotosports/fury
.PHONY: format
